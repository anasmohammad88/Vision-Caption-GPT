...
from dataset.grounding import GroundingVocab
...
def main():
    ...
    train_images, valid_images = train_test_split(
        image_names, test_size=0.2, random_state=42, shuffle=True
    )
    ...
    # 3. Vocabulary (caption vocab)
    print("3. Build vocabulary...")
    train_captions = [c for image in train_images for c in image_to_captions[image]]
    vocab = Vocabulary(Config.VOCAB_SIZE, Config.MIN_FREQUENCY)
    vocab.build_vocabulary(train_captions, tokenizer)
    
    # Save vocabulary
    with open(CHECKPOINT_DIR/"vocabulary.pkl", "wb") as file:
    ...
    # 3b. Grounding concept vocab
    grounding_vocab = None
    if Config.USE_GROUNDING:
        print("3b. Build grounding concept vocab...")
        grounding_vocab = GroundingVocab(num_concepts=Config.NUM_CONCEPTS)
        grounding_vocab.build(image_to_captions, train_images)
        print(f"GROUNDING CONCEPTS: {len(grounding_vocab)}")
        print(f"SAMPLE CONCEPTS: {list(grounding_vocab.word_to_idx)[:15]}")
    ...
    train_loader = create_dataloader(train_images, preprocessor, image_to_captions, TRAIN_TRANSFORM, Config.BATCH_SIZE, shuffle=True, random_caption=True, grounding_vocab=grounding_vocab)
    ...
    num_concepts = len(grounding_vocab) if grounding_vocab is not None else 0
    vision_caption_model = VisionCaptionModel(
        ...
        num_concepts=num_concepts # Enables the grounding head
    ).to(device)
    ...
    optimizer = optim.AdamW(params=vision_caption_model.parameters(), lr=Config.LEARNING_RATE, weight_decay=Config.WEIGHT_DECAY)
